...
GeotiffParser.prototype = {
    ...
    */
    ...
    /* getRGBAPixelValue
    *  This function is the default one , you should use this function in order to draw the image into a canvas
    *  If you have a multiband image , you should define how to combine bands in order to obtain a RGBA value
    */
   ...
   */
    ...
    */
    ...
    toRGB: function (xmin, ymin, xmax, ymax, vmin, vmax) {
        var pixrgba = new Uint8Array(1 << (Math.floor(Math.log(xmax * ymax * 4) / Math.log(2))));
        var currentPixel = null;
        for (var y = ymin; y < ymax; y++) {
            for (var x = xmin; x < xmax; x++) {
                var pixSample = this.getPixelValueOnDemand(x, y);
                if (pixSample !== 'undefined') {
                    //console.log("pixSample");
                    //console.log(pixSample);
                    if (vmin !== 'undefined' && vmax !== 'undefined') {
                        currentPixel = this.getMinMaxPixelValue(pixSample, vmin, vmax);
                        pixrgba[(y * xmax + x) * 4] = currentPixel[0];//255;
                        pixrgba[(y * xmax + x) * 4 + 1] = currentPixel[1];//255;
                        pixrgba[(y * xmax + x) * 4 + 2] = currentPixel[2];//255;
                        pixrgba[(y * xmax + x) * 4 + 3] = 255;//currentPixel[3];
                    } else {
                        currentPixel = this.getRGBAPixelValue(pixSample);
                        pixrgba[(y * xmax + x) * 4] = currentPixel[0];//255;
                        pixrgba[(y * xmax + x) * 4 + 1] = currentPixel[1];//255;
                        pixrgba[(y * xmax + x) * 4 + 2] = currentPixel[2];//255;
                        pixrgba[(y * xmax + x) * 4 + 3] = 255;//currentPixel[3];
                    ...
                    pixrgba[(y * xmax + x) * 4] = 255;
                    pixrgba[(y * xmax + x) * 4 + 1] = 0;
                    pixrgba[(y * xmax + x) * 4 + 2] = 0;
                    pixrgba[(y * xmax + x) * 4 + 3] = 1;
                }
            }
        }
        console.log("pixrgba");
